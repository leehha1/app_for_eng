...
class DataBase:
    ...
    def from_csv_to_list(path_to_words: str) -> list[list]:
        ...
        :param path_to_words: path to csv file
        :return: A list of lists, where the first list is the names of the columns,
...
class TranslatedWords(DataBase):
        ...
        if counter > 1:
            for line in list_df[1]:
                ...
                if q_where != "":
                    ...
                    res = self.query_to_db("execute", query_select)

                if not res:
                    query_insert = f"INSERT INTO words ({', '.join(list_df[0])}) VALUES(?,?,?)"
                    self.query_to_db("execute", query_insert, line)
                else:
    ...
    def delete_duplicates(self):
        ...
        res = self.query_to_db("execute", query)
        return res

    def get_random_words(self, num_words=1):
